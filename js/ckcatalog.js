...
CKCatalog.tabs['discoverability'] = (function() {
  ...
  var discoverUserIdentityWithEmailAddressSample = {
    ...
    sampleCode: function demoDiscoverUserIdentityWithEmailAddress(emailAddress) {
      ...
      return container.discoverUserIdentityWithEmailAddress(emailAddress).then(function(response) {
        if (response.hasErrors) {
  ...
  var discoverUserIdentityWithUserRecordNameSample = {
    ...
    sampleCode: function demoDiscoverUserIdentityWithUserRecordName(userRecordName) {
      ...
      return container.discoverUserIdentityWithUserRecordName(userRecordName).then(function(response) {
        if (response.hasErrors) {
  ...
  return [fetchUserIdentitySample, discoverAllUserIdentitiesSample, discoverUserIdentityWithEmailAddressSample, discoverUserIdentityWithUserRecordNameSample];
...
CKCatalog.tabs['query'] = (function() {
  ...
  var querySample = {
    ...
    sampleCode: function demoPerformQuery(databaseScope, zoneName, ownerRecordName, recordType, desiredKeys, sortByField, ascending, latitude, longitude, filters) {
      var container = CloudKit.getDefaultContainer();
      var database = container.getDatabaseWithDatabaseScope(CloudKit.DatabaseScope[databaseScope]);
      ...
      // Execute the query.
      return database.performQuery(query, options).then(function(response) {
        if (response.hasErrors) {
  ...
  var renderZones = function(zones) {
    ...
    if (zones.length === 0) {
      ...
      zones.forEach(function(zone) {
        table.appendRow([zone.zoneID, zone.atomic, zone.syncToken]);
      })
    }
    content.appendChild(heading);
    content.appendChild(table.el);
...
CKCatalog.tabs['records'] = (function() {
  ...
  var saveRecordSample = {
    ...
    sampleCode: function demoSaveRecords(databaseScope, recordName, recordChangeTag, recordType, zoneName, forRecordName, forRecordChangeTag, publicPermission, ownerRecordName, participants, parentRecordName, fields, createShortGUID) {
      var container = CloudKit.getDefaultContainer();
      var database = container.getDatabaseWithDatabaseScope(CloudKit.DatabaseScope[databaseScope]);
      ...
      return database.saveRecords(record, options).then(function(response) {
        if (response.hasErrors) {
  ...
  var deleteRecordSample = {
    ...
    sampleCode: function demoDeleteRecord(databaseScope, recordName, zoneName, ownerRecordName) {
      ...
      return database.deleteRecords(recordName, options).then(function(response) {
        if (response.hasErrors) {
  ...
  var fetchRecordSample = {
    ...
    sampleCode: function demoFetchRecord(databaseScope, recordName, zoneName, ownerRecordName) {
      ...
      return database.fetchRecords(recordName, options).then(function(response) {
        if (response.hasErrors) {
...
CKCatalog.tabs['sync'] = (function() {
  ...
  var renderZones = function(databaseScope, zones, syncToken, moreComing) {
    ...
    if (zones.length === 0) {
      ...
      zones.forEach(function(zone) {
        zonesTable.appendRow([zone.zoneID, zone.syncToken, zone.atomic, zone.deleted]);
      });
    }
    ...
    sampleCode: function demoFetchRecordZoneChanges(databaseScope, zoneName, ownerRecordName, syncToken) {
      ...
      return database.fetchRecordZoneChanges(args).then(function(response) {
        if (response.hasErrors) {
          ...
          return renderRecords(databaseScope, zoneName, ownerRecordName, records, newSyncToken, moreComing);
        }
      });
    }
...
CKCatalog.tabs['sharing'] = (function() {
  ...
  var resolveShortGUIDSample = {
    ...
    sampleCode: function demoFetchRecordInfos(shortGUID) {
      ...
      return container.fetchRecordInfos(shortGUID).then(function(response) {
        if (response.hasErrors) {
  ...
  var acceptShareSample = {
    ...
    sampleCode: function demoAcceptShares(shortGUID) {
      ...
      return container.acceptShares(shortGUID).then(function(response) {
        if (response.hasErrors) {
  ...
  var shareWithUISample = {
    ...
    sampleCode: function demoShareWithUI(databaseScope, recordName, zoneName, ownerRecordName, shareTitle, supportedAccess, supportedPermissions) {
      var container = CloudKit.getDefaultContainer();
      var database = container.getDatabaseWithDatabaseScope(CloudKit.DatabaseScope[databaseScope]);
...
CKCatalog.tabs['subscriptions'] = (function() {
  ...
  var saveSubscriptionSample = {
    ...
    sampleCode: function demoSaveSubscriptions(subscriptionType, zoneName, recordType, firesOn, filterBy) {
      var container = CloudKit.getDefaultContainer();
      var privateDB = container.privateCloudDatabase;
  ...
  var addNotificationListenerSample = {
    ...
    sampleCode: function demoRegisterForNotifications() {
      ...
      function renderNotification(notification) {
        appendNotificationToTable(notification.notificationID, notification.notificationType, notification.subscriptionID, notification.zoneID);
      }
